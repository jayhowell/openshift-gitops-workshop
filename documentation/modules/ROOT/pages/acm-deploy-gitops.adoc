...
Let’s just do *one more step*: In the bastion machine, run this command
below to make sure our lab user has the right permissions to create
policies with policy generator:
...
** *URL*:
https://github.com/levenhagen/demo-policygenerator.git[https://github.com/levenhagen/demo-policygenerator.git,window='_blank'] +
* *Branch*: main +
...
didn’t stall OpenShift GitOps manually through the Operator Hub, as
stated in the previous section, the Governance would report a violation
❗.
